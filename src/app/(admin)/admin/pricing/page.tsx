...
import { marketingPackages, photoshootPackages as staticPhotoshoot, videoPackages as staticVideo } from '@/data/staticData'
...
export default async function AdminPricingPage() {
  ...
  try {
    const [mRows, pRows, vRows] = await Promise.all([
      ...
      prisma.videoPackage.findMany({ orderBy: { sortOrder: 'asc' } }),
    ])
    ...
    <>
      <div className="admin-page-header">
        <div>
          ...
          <p className="admin-page-sub">{mPkgs.length} marketing · {pPkgs.length} photoshoot · {vPkgs.length} video packages</p>
        </div>
        <div style={{ display: 'flex', gap: '0.75rem' }}>
          <Link href="/admin/pricing/photoshoot/new" className="admin-btn admin-btn--outline">+ Photoshoot Package</Link>
          <Link href="/admin/pricing/video/new" className="admin-btn admin-btn--outline">+ Video Package</Link>
