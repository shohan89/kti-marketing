...
import { DEPT_COLORS } from '@/data/staticData'
import type { JobListing } from '@/data/staticData'
    ...
    <article className="job-card">
      ...
      <div className="job-card__meta">
        <span className="job-card__location">
          <svg width="13" height="13" viewBox="0 0 16 16" fill="none" aria-hidden="true"><path d="M8 1.5A4.5 4.5 0 0 0 3.5 6c0 3.5 4.5 8.5 4.5 8.5S12.5 9.5 12.5 6A4.5 4.5 0 0 0 8 1.5Zm0 6a1.5 1.5 0 1 1 0-3 1.5 1.5 0 0 1 0 3Z" fill="currentColor" /></svg>
          {job.location}
        </span>
    ...
    <main className="careers-page">
      <section className="careers-hero">
        <div className="container">
          ...
          <h1 className="careers-hero__title fade-up-1">Build Something<br /><span className="accent">Great. Together.</span></h1>
      ...
      <section className="careers-listings">
        <div className="container">
          <div className="careers-tabs" role="tablist" aria-label="Filter jobs by department">
            {FILTERS.map(dept => (
              <button key={dept} role="tab" aria-selected={filter === dept} className={`careers-tab${filter === dept ? ' careers-tab--active' : ''}`} onClick={() => setFilter(dept)}>
                {dept}<span className="careers-tab__count">{countFor(dept)}</span>
              </button>
            ))}
          </div>
          {visible.length > 0 ? (
            <div className="careers-grid">{visible.map(job => (<JobCard key={job.slug} job={job} />))}</div>
          ) : (
            <div className="careers-empty"><p>No openings in <strong>{filter}</strong> right now. Check back soon.</p></div>
          )}
        </div>
      </section>
