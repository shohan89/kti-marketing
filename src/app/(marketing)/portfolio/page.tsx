...
export async function generateMetadata(): Promise<Metadata> {
  ...
  return buildMetadata(seo, { title: 'Portfolio — Results That Prove We Deliver', description: 'Real campaigns. Real outcomes. Every project is built on verified business results — revenue, leads, ROAS, and retention.' })
}
          ...
          <div className="cs-featured__card">
            <div className="cs-featured__left">
              ...
              <div className="cs-featured__actions">
                <Link href={`/portfolio/${featured.slug}`} className="btn btn-primary">Read Full Case Study →</Link>
          ...
          <div className="cs-grid">
            {rest.map(({ slug, tag, category, client, title, challenge, solution, metrics, duration, services }, i) => (
              <div className="cs-card reveal" key={slug} style={{ '--reveal-delay': `${i * 0.1}s` } as React.CSSProperties}>
                ...
                <Link href={`/portfolio/${slug}`} className="cs-card__cta">Read Full Case Study →</Link>
              </div>
            ))}
          </div>
        </div>
