      ...
      <section className="cs-hero">
        ...
        <div className="container cs-hero__inner">
          ...
          <h1 className="cs-hero__title">
            Results That Prove<br />
            <span className="cs-hero__accent">We Deliver.</span>
          </h1>
          <p className="cs-hero__sub">
            We don't hide behind impressions and engagement rate graphs.
            Every case study below is built on real business outcomes —
            revenue, leads, ROAS, and retention.
          </p>
          <div className="cs-hero__stats">
            {[
              ...
              <div className="cs-hero__stat" key={label}>
                <span className="cs-hero__stat-num">{num}</span>
                <span className="cs-hero__stat-lbl">{label}</span>
              </div>
            ))}
          </div>
        </div>
          ...
          <div className="cs-featured__card">
            <div className="cs-featured__left">
              ...
              <div className="cs-featured__blocks">
                <div className="cs-block">
                  <h4 className="cs-block__label">The Challenge</h4>
                  <p className="cs-block__body">{featured.challenge}</p>
                </div>
                <div className="cs-block">
                  <h4 className="cs-block__label">Our Approach</h4>
                  <p className="cs-block__body">{featured.solution}</p>
                </div>
              </div>
              <div className="cs-featured__services">
                {featured.services.map(s => (
                  <span className="cs-service-pill" key={s}>{s}</span>
                ))}
              </div>
              <div className="cs-featured__actions">
                ...
                <Link to="/contact" className="btn btn-outline cs-featured__cta">
            ...
            <div className="cs-featured__right">
              <div className="cs-metrics-panel">
                ...
                <div className="cs-metrics-panel__duration">
                  <span>Duration</span>
                ...
                <div className="cs-metrics-grid">
                  {featured.metrics.map(({ num, label }) => (
                    <div className="cs-metric" key={label}>
                      <span className="cs-metric__num">{num}</span>
        ...
        <div className="container">
          <div className="cs-grid-header reveal">
            ...
            <h2 className="cs-grid-title">
              Every Client.<br />
              <span className="accent">Every Outcome.</span>
            </h2>
          </div>
          <div className="cs-grid">
            {rest.map(({ slug, tag, category, client, title, challenge, solution, metrics, duration, services }, i) => (
              <div className="cs-card reveal" key={slug} style={{ '--reveal-delay': `${i * 0.1}s` }}>
                ...
                <div className="cs-card__blocks">
                  <div className="cs-block">
                    <h4 className="cs-block__label">Challenge</h4>
                    <p className="cs-block__body">{challenge}</p>
                  </div>
                  <div className="cs-block">
                    <h4 className="cs-block__label">Approach</h4>
                    <p className="cs-block__body">{solution}</p>
                  </div>
                </div>
                <div className="cs-card__metrics">
                  {metrics.map(({ num, label }) => (
                    <div className="cs-metric cs-metric--sm" key={label}>
                      <span className="cs-metric__num">{num}</span>
                      <span className="cs-metric__label">{label}</span>
                    </div>
                  ))}
                ...
                <div className="cs-card__footer">
                  <div className="cs-card__services">
                    {services.map(s => (
                      <span className="cs-service-pill cs-service-pill--sm" key={s}>{s}</span>
                    ))}
                  </div>
                  <span className="cs-card__duration">{duration}</span>
                </div>
                <Link to={`/case-studies/${slug}`} className="cs-card__cta">
                  Read Full Case Study →
                </Link>
              </div>
            ))}
